...
class VC_Config {
	...
	public function write( $config ) {
		...
		$config_dir = WP_CONTENT_DIR . '/vc-config';
	...
	public function get() {
		...
		$config = get_option( 'vc_voyar_cache', $this->get_defaults() );
	...
	public function verify_file_access() {
		...
		// If the cache and/or cache/voyar-cache directories exist, make sure it's writeable
		if ( @file_exists( untrailingslashit( WP_CONTENT_DIR ) . '/cache' ) ) {
			if ( ! $this->_is_dir_writable( untrailingslashit( WP_CONTENT_DIR ) . '/cache' ) ) {
				return false;
			}

			if ( @file_exists( untrailingslashit( WP_CONTENT_DIR ) . '/cache/voyar-cache' ) ) {
				if ( ! $this->_is_dir_writable( untrailingslashit( WP_CONTENT_DIR ) . '/cache/voyar-cache' ) ) {
					return false;
				}
			}
		}

		// If the vc-config directory exists, make sure it's writeable
		if ( @file_exists( untrailingslashit( WP_CONTENT_DIR ) . '/vc-config' ) ) {
			if ( ! $this->_is_dir_writable( untrailingslashit( WP_CONTENT_DIR ) . '/vc-config' ) ) {
				return false;
			}
		}
	...
	public function clean_up() {
		...
		$folder = untrailingslashit( WP_CONTENT_DIR ) . '/vc-config';

		delete_option( 'vc_voyar_cache' );
