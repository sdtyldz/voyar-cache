...
if ( empty( $batcache ) && ! empty($GLOBALS['vc_config'] ) && isset( $GLOBALS['vc_config']['page_cache_length'] ) ) {
	$max_age = $GLOBALS['vc_config']['page_cache_length'] * 60;
...
if ( ! empty( $GLOBALS['vc_config']['cache_exception_urls'] ) ) {
	$exceptions = preg_split( '#(\n|\r)#', $GLOBALS['vc_config']['cache_exception_urls'] );
...
if ( isset( $batcache->cache['time'] )  // We have cache
	...
	|| ( $batcache->do && $batcache->use_stale )    )
) {
