...
if ( ! empty( $_COOKIE ) ) {
	...
	if ( ! empty( $_COOKIE['vc_commented_posts'] ) ) {
		foreach ( $_COOKIE['vc_commented_posts'] as $path ) {
...
if ( ! empty( $GLOBALS['vc_config']['advanced_mode'] ) && ! empty( $GLOBALS['vc_config']['cache_exception_urls'] ) ) {
	$exceptions = preg_split( '#(\n|\r)#', $GLOBALS['vc_config']['cache_exception_urls'] );

	$regex = ( ! empty( $GLOBALS['vc_config']['enable_url_exemption_regex'] ) ) ? true : false;

	foreach ( $exceptions as $exception ) {
		if ( vc_url_exception_match( $exception, $regex ) ) {
			// Exception match
			return;
		}
	}
