 ...
 * @return string
 */
function vc_cache( $buffer, $flags ) {
	...
	if ( ! $filesystem->exists( untrailingslashit( WP_CONTENT_DIR ) . '/cache/voyar-cache' ) ) {
		if ( ! $filesystem->mkdir( untrailingslashit( WP_CONTENT_DIR ) . '/cache/voyar-cache' ) ) {
			// Can not cache!
			return $buffer;
		}
	}

	$buffer = apply_filters( 'vc_pre_cache_buffer', $buffer );

	$url_path = vc_get_url_path();

	$dirs = explode( '/', $url_path );

	$path = untrailingslashit( WP_CONTENT_DIR ) . '/cache/voyar-cache';
	...
	if ( preg_match( '#</html>#i', $buffer ) ) {
		$buffer .= "\n<!-- Cache served by Voyar Cache - Last modified: " . gmdate( 'D, d M Y H:i:s', $modified_time ) . " GMT -->\n";
	}

	if ( ! empty( $GLOBALS['vc_config']['enable_gzip_compression'] ) && function_exists( 'gzencode' ) ) {
	...
	if ( function_exists( 'ob_gzhandler' ) && ! empty( $GLOBALS['vc_config']['enable_gzip_compression'] ) ) {
 ...
 * @since 1.0
 */
function vc_serve_cache() {
	$file_name = 'index.html';

	if ( function_exists( 'gzencode' ) && ! empty( $GLOBALS['vc_config']['enable_gzip_compression'] ) ) {
		$file_name = 'index.gzip.html';
	}

	$path = rtrim( WP_CONTENT_DIR, '/' ) . '/cache/voyar-cache/' . rtrim( vc_get_url_path(), '/' ) . '/' . $file_name;
 ...
 * @return boolean
 */
function vc_url_exception_match( $exception, $regex = false ) {
