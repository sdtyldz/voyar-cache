...
class VC_Advanced_Cache {
	...
	public function set_comment_cookie_exceptions( $comment, $user ) {
		...
		if ( ! empty( $config['enable_page_caching'] ) && empty( $config['enable_in_memory_object_caching'] ) ) {
			...
			setcookie( 'vc_commented_posts[' . $post_id . ']', parse_url( get_permalink( $post_id ), PHP_URL_PATH ), ( time() + HOUR_IN_SECONDS * 24 * 30 ) );
		}
	}
	...
	public function purge_post_on_comment_status_change( $comment_ID, $comment_status ) {
		...
		if ( ! empty( $config['enable_page_caching'] ) && empty( $config['enable_in_memory_object_caching'] ) ) {
			...
			$path = untrailingslashit( WP_CONTENT_DIR ) . '/cache/voyar-cache/' . preg_replace( '#https?://#i', '', get_permalink( $post_id ) );
	...
	public function print_notice() {
		...
		if ( defined( 'VC_ADVANCED_CACHE' ) && VC_ADVANCED_CACHE ) {
		...
		<div class="error">
			<p>
				<?php if ( $config_file_bad ) : ?>
					<?php esc_html_e( 'wp-config.php içerisinde define("WP_CACHE", true); satırı bulunamadı.', 'voyar-cache' ); ?>
				<?php endif; ?>

				<?php if ( $advanced_cache_file_bad ) : ?>
					<?php esc_html_e( 'wp-content/advanced-cache.php dosyası değiştirildi veya silindi.', 'voyar-cache' ); ?>
				<?php endif; ?>

				<?php esc_html_e( 'Voyar Cache eklentisi kullanılamaz durumda.', 'voyar-cache' ); ?>
	...
	public function clean_up() {
		...
		$folder = untrailingslashit( WP_CONTENT_DIR )  . '/cache/voyar-cache';
	...
	public function write() {
		...
		if ( ! empty( $config['enable_page_caching'] ) ) {
			...
			"\n\r" . "define( 'VC_ADVANCED_CACHE', true );" .
			...
			"\n\r" . "if ( empty( \$GLOBALS['vc_config'] ) || empty( \$GLOBALS['vc_config']['enable_page_caching'] ) ) { return; }" .
	...
	public function toggle_caching( $status ) {
		...
		array_unshift( $config_file, '<?php', "define( 'WP_CACHE', $status_string ); // Voyar Cache" );
